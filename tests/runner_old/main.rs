...
async fn run_with_empty_program() -> Result<(), Error> {
    compile_and_run(Path::new("tests/runner_old/empty"), true).await
}

#[tokio::test]
async fn run_with_syntax_specificities() -> Result<(), Error> {
    compile_and_run(Path::new("tests/runner_old/syntax"), true).await
}

#[tokio::test]
async fn run_with_exprs() -> Result<(), Error> {
    compile_and_run(Path::new("tests/runner_old/exprs"), true).await
}

#[tokio::test]
async fn run_with_items() -> Result<(), Error> {
    compile_and_run(Path::new("tests/runner_old/items"), true).await
}

#[tokio::test]
async fn run_with_aliasing() -> Result<(), Error> {
    compile_and_run(Path::new("tests/runner_old/aliasing"), false).await
}

#[tokio::test]
async fn run_with_prelude() -> Result<(), Error> {
    compile_and_run(Path::new("tests/runner_old/prelude"), true).await
}
