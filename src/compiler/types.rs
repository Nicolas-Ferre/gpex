...
impl<'item, 'index> TypeResolver<'item, 'index> {
    ...
    pub(crate) fn var_type(&self, node: &VarDefinition) -> Type<'index> {
        self.expr_type(&node.default_value)
    }

    pub(crate) fn param_type(&self, node: &Param) -> Type<'index> {
        self.expr_as_type(&node.type_)
    }

    pub(crate) fn fn_type(&self, node: &FnDefinition) -> Type<'index> {
    ...
    pub(crate) fn expr_type(&self, node: &Expr) -> Type<'index> {
        match node {
    ...
    pub(crate) fn expr_as_type(&self, node: &Expr) -> Type<'index> {
    ...
    fn source_type(&self, node_id: u64) -> Type<'index> {
    ...
    fn item_type(&self, node: ItemRef<'_>) -> Type<'index> {
        match node {
