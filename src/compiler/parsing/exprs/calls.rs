...
pub(crate) fn parse_unary<'context>(
    ...
    context.force_parse_any_error();
    let operand = super::parse_operand(context, stop_excluded_parser)?;
...
pub(super) fn from_binary(
    ...
    Call {
        ...
        name,
        args: vec![unnamed(left_operand), unnamed(right_operand)],
    }
}

pub(super) fn from_uniform_syntax(receiver: Expr, call: Call) -> Call {
    let receiver_span = receiver.span();
    let mut args = vec![unnamed(receiver)];
...
fn parse_arg<'context>(context: &mut ParseContext<'context>) -> Result<Arg, ParseError<'context>> {
    context.parse_any(&[&parse_named, &parse_unnamed])
}

fn parse_named<'context>(
    context: &mut ParseContext<'context>,
) -> Result<Arg, ParseError<'context>> {
    let name_span = Span::parse_pattern(context, IDENT_PATTERN)?;
